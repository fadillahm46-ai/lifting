// ==========================================
// BACKEND API CRANE TRUCK MONITORING (FINAL STABLE)
// ==========================================
...
function doPost(e) {
  try {
    ...
    let result;
    // RUTE METODE POST PENUH (Anti-Blokir / Anti-Redirect Drop)
    ...
    else throw new Error("Aksi POST tidak valid");

    // Memberikan label "status: success" manual agar frontend membacanya dengan aman
    if (!result.status) result.status = "success";

    return ContentService.createTextOutput(JSON.stringify(result)).setMimeType(ContentService.MimeType.JSON);
  } catch (error) {
    return ContentService.createTextOutput(JSON.stringify({ success: false, status: "error", message: error.toString() })).setMimeType(ContentService.MimeType.JSON);
  }
}
...
function doGet(e) {
  return ContentService.createTextOutput(JSON.stringify({
    status: "success",
    message: "API Aktif. Sistem Monitoring Berjalan. Harap gunakan metode POST dari aplikasi."
  ...
  if (orderSheet) {
    ...
    if (data.length > 1) {
      ...
      for (let i = 1; i < data.length; i++) {
        ...
        // PENCEGAH ERROR: Lewati baris yang kosong (bolong)
        if (!row[0]) continue;
...
function saveOrder(payload) {
  ...
  const newRow = [
    idOrder, now, payload.nama, payload.wa, payload.perusahaan, payload.departemen, payload.section, payload.tanggal, payload.shift, payload.waktu, payload.durasi, payload.lokasi, payload.tujuan, payload.deskripsi, payload.foto,
    "Menunggu Validasi", "", "", "", "", "", "", "", ""
  ];
...
function updateJobRecord(payload) {
  ...
  let targetRow = -1;
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === payload.id) { targetRow = i + 1; break; }
  }
  ...
  if (payload.isResuming) {
    ...
    const newRow = [
      ...
      payload.tglReq || oldData[7], payload.shift || oldData[8], payload.waktuReq || oldData[9], payload.durasiReq || oldData[10],
      oldData[11], oldData[12], payload.deskripsiReq || oldData[13], oldData[14], payload.status, payload.unit || "", payload.gl || "", payload.operator || "", payload.rigger || "", "", "", "", ""
    ];
    sheet.appendRow(newRow);
    return { success: true };
  }
...
function deleteMasterItem(category, value) {
  ...
  const data = sheet.getDataRange().getValues();
  for (let i = 1; i < data.length; i++) {
    if (data[i][0] === category && data[i][1] === value) { sheet.deleteRow(i + 1); return { success: true }; }
  }
  return { success: false, message: "Data tidak ditemukan" };
}
