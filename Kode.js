...
// Fungsi utama untuk menangani request dari Vercel (Frontend)
function doPost(e) {
  try {
...
// Menangani akses langsung dari Browser (Mencegah error 'doGet')
function doGet(e) {
  ensureSheetsExist();
...
function getAppData() {
  ...
  return { master: masterData, orders: orders.reverse() }; // Reverse agar data terbaru di atas
}
...
function updateJobRecord(payload) {
  ...
  // Jika validasi pertama kali (isResuming) -> buat baris baru untuk split job
  if (payload.isResuming) {
    ...
    const newRow = [
      ...
      payload.operator || "",
      payload.rigger || "",
      "", "", "", "" // Waktu start/end dikosongkan
    ];
    sheet.appendRow(newRow);
    return { success: true, message: "Job berhasil divalidasi dan di-split." };
  }
  
  // Update data langsung jika tidak resume
